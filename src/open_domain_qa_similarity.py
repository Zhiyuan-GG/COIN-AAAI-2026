...
parser.add_argument('--similarity-model',
                    default=r'E:\cache_model\stsb-roberta-large',
...
for generation in tqdm.tqdm(generations):
    ...
    for i, gen in enumerate(sampled_generated_texts):
        ...
        for j, gen_temp in enumerate(sampled_generated_texts):
            ...
            if j == i:
                similarity_dict[i].append(1.0)
            elif j > i:
                similarity_dict[i].append(similarity_model.predict([qa_1, qa_2]))
            elif j < i:
                similarity_dict[i].append(similarity_dict[j][i])
        print(f'id-{id} --- similarity-{i}: ', similarity_dict[i])
    id_to_similarity[id] = [item for item in similarity_dict.values()]

# save
with open(f'{run_name}/sampled_generations_similarity.pkl', 'wb') as record_file:
    pickle.dump(id_to_similarity, record_file)
print('Record saved to ', f'{run_name}/sampled_generations_similarity.pkl')