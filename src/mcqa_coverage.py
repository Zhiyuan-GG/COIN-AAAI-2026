...
alpha_list = np.linspace(0.15, 0.35, 11).tolist()
mean_list = []
std_list = []
for alpha in alpha_list:
    multi_check_coverage_list = []
        ...
        if t_hat is None:
            ...
            for test_data in test_set:
                ...
                if uncertainty <= t_hat:
                    coverage += 1
            multi_check_coverage_list.append(coverage)

    multi_check_coverage_numpy = np.array(multi_check_coverage_list)
    mean = np.mean(multi_check_coverage_numpy)
    std = np.std(multi_check_coverage_numpy)
